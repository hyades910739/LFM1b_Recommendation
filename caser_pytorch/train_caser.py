...
class Recommender(object):
    ...
    def fit(self, train, test, verbose=False):
        ...
        #hyades
        seqids = train.sequences.seq_ids
        #
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            self._net.train()
            # hyades
            users, sequences, targets, seqids = shuffle(users,
                                                        sequences,
                                                        targets,
                                                        seqids)
            #在每個subseq 依據user 評分過的item，隨機找n個來當negative samples
    ...
    def _generate_negative_samples(self, users, interactions, n):
        ...
        if not self._candidate:
            ...
            for user, row in enumerate(train): # iter csr by row
                self._candidate[user] = list(set(all_items) - set(row.indices)) # row.indice = cols which are not 0
    ...
    def predict_subseq(self, seqid, user_id, item_ids=None):
        """
        Make predictions for evaluation: given a user id, it will
        first retrieve the test sequence associated with that user
        and compute the recommendation scores for items.
        ...
        sequence = self.test_sequence.sequences[seqid, :]
        sequence = np.atleast_2d(sequence)
...
if __name__ == '__main__':
    ...
    # data arguments
    parser.add_argument('--train_root', type=str, default='lfm_1b/train25')
    parser.add_argument('--test_root', type=str, default='lfm_1b/test25')
    ...
    # load dataset
    #hyades
    train = MusicInteraction(config.train_root)
    # transform triplets to sequence representation
    train.to_sequence(config.L, config.T)
    #hyades
    test = MusicInteraction(config.test_root)#,train.num_items)
