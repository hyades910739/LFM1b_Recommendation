...
class Interactions(object):
    ...
    def tocoo(self):
        ...
        return sp.coo_matrix((data, (row, col)),
                             shape=(self.num_users, self.num_items)) #(row,col)處為1，其餘為0
    ...
    def to_sequence(self, sequence_length=5, target_length=1):
        ...
        for k, v in self.item_map.items():
            self.item_map[k] = v + 1
        self.item_ids = self.item_ids + 1 #numpy array
        self.num_items += 1
        ...
        '''
            for example : user 1 with seq: [1,2,3,4,5,6,7,8],seq_len = 4,item_len=2
            then:
            test_seq = [5,6,7,8]
            sequences = [[3,4,5,6],[2,3,4,5],[1,2,3,4]]
            sequces_targets = [[7,8],[6,7],[5,6]]
        '''
        _uid = None
        for i, (uid,
