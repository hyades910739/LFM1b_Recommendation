...
def evaluate_ranking(model, test, train=None, k=10):
    ...
    """
    seqid_userid = test.seqid_userid
    test = test.tocsr()
    ...
    apks = list()
    #hyades
    for seqid, row in enumerate(test):
        #print(f"predicting subseq with seqid{seqid}")
        if not len(row.indices):
            continue
        #hyades
        user_id = seqid_userid[seqid]

        predictions = -model.predict_subseq(seqid,user_id)
